...
async def read_all_by_user(request: Request, db: Session = Depends(get_db), user=Depends(get_current_user)):
    if user is None:
    ...
    completed_habit_ids = db.query(models.Complete.habit_id) \
    ...
    return templates.TemplateResponse("home.html", {"request": request, "habits": habits, "user": user,
...
async def add_new_habit(request: Request):
    ...
    return templates.TemplateResponse("add-habit.html", {"request": request, "user": user})


# Endpoint to create a new habit
@router.post("/add-habit", response_class=HTMLResponse)
async def create_habit(title: str = Form(...), description: str = Form(...),
                       priority: int = Form(...), db: Session = Depends(get_db),
                       days: List[int] = Form(...), user=Depends(get_current_user)):
    if user is None:
        return RedirectResponse(url="/auth", status_code=status.HTTP_302_FOUND)
...
async def edit_habit(request: Request, habit_id: int, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("edit-habit.html", {"request": request, "habit": habit, "user": user})


# Endpoint to update an existing habit
@router.post("/edit-habit/{habit_id}", response_class=HTMLResponse)
async def edit_habit_commit(request: Request, habit_id: int, title: str = Form(...),
...
async def delete_habit(habit_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    if user is None:
        return RedirectResponse(url="/auth", status_code=status.HTTP_302_FOUND)

    habit_model = db.query(models.Habits).filter(models.Habits.id == habit_id) \
        .filter(models.Habits.owner_id == user.get("id"))
...
async def complete_habit(habit_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    if user is None:
        return RedirectResponse(url="/auth", status_code=status.HTTP_302_FOUND)
...
async def undo_completion(habit_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):

    if user is None:
        return RedirectResponse(url="/auth", status_code=status.HTTP_302_FOUND)

    completion_entry = db.query(models.Complete).filter(
...
async def list_all_habits(request: Request, db: Session = Depends(get_db), user=Depends(get_current_user)):
    if user is None:
        return RedirectResponse(url="/auth", status_code=status.HTTP_302_FOUND)

    habits = db.query(models.Habits).filter(models.Habits.owner_id == user.get("id")).order_by(
        desc(models.Habits.priority)).all()

    return templates.TemplateResponse("all-habits.html", {"request": request, "habits": habits, "user": user})
